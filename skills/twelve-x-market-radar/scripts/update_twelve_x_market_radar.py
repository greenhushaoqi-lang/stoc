#!/usr/bin/env python3
"""Update the local twelve-account X market radar skill memory."""
...
def http_get(url: str, headers: dict[str, str] | None = None, timeout: int = 20) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 twelve-x-market-radar/1.0",
            **(headers or {}),
        },
    )
...
def load_import_file(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if handle_key not in ACCOUNTS:
            continue
        out.append(normalize_item(row, handle_key, f"import:{path.name}"))
    return out
...
def build_latest_markdown(rows: list[dict[str, Any]], source_status: list[str]) -> str:
    ...
    lines = [
        "# Twelve X Market Radar - Latest Learned State",
        "",
        f"Last update: {stamp}",
    ...
    lines += [
        ...
        "- Use isolated account ideas as leads only; do not convert them into trade conclusions without independent verification.",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Update twelve tracked X accounts for market narrative research.")
