#!/usr/bin/env python3
"""Update the local selected-account X market radar skill memory."""
...
def http_get(url: str, headers: dict[str, str] | None = None, timeout: int = 20) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 zhanru-astock-x-radar/1.0",
            **(headers or {}),
        },
    )
...
def build_latest_markdown(rows: list[dict[str, Any]], source_status: list[str]) -> str:
    ...
    lines = [
        "# Zhanru AStock X Radar - Latest Learned State",
        "",
        f"Last update: {stamp}",
    ...
    if rows:
        for key in ("cashtags", "hashtags", "stock_names", "keywords"):
            ...
            lines.append(f"- {key}: {', '.join(values[:50]) if values else 'none detected'}")
    ...
    for handle in ACCOUNTS:
        ...
        lines.append(f"- keywords: {', '.join(terms['keywords'][:25]) if terms['keywords'] else 'none detected'}")
        lines.append(f"- stock_names: {', '.join(terms['stock_names'][:25]) if terms['stock_names'] else 'none detected'}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Update selected tracked X accounts for market narrative research.")
