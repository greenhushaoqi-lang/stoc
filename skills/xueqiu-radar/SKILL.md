...
雪球(xueqiu.com)有 WAF 反爬,**匿名访问被拦截**(只返回JS挑战页/403),连昵称都解析不了。必须提供**登录态 cookie(含 xq_a_token)**。脚本按优先级查找:
- 方式1:环境变量 `XUEQIU_COOKIE="整段cookie"`
- 方式2(**最易创建,推荐**):普通文件 `C:\Users\lixue\Desktop\Glozin\xueqiu_cookie.txt`
- 方式3:`%USERPROFILE%\xueqiu_cookie.txt` 或 `%USERPROFILE%\.xueqiu_cookie.txt`
